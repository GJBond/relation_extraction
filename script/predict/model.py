...
class TFMyBertModel(tf.keras.models.Model):
    ...
    # return start index of predicate or objdect in the input.
    ...
    # convert spo from inputs to embedding position info
    ...
    def call(self, inputs, **kwargs):
        ...
        viterbi_decoded, potentials, sequence_length, chain_kernel = self.crf(
            output_layer, input_mask)

        viterbi_decoded *= input_mask
        ner_predict = viterbi_decoded
        ...
        # get embedding position info of subject, predicate and object
        for pos,sub,token in zip(ner_predict,subj_token,input_ids):
            ...
            f_pre_start = self.get_index(pos,5)  # 5 is B-属性
            f_obj_start = self.get_index(pos,8)  # 8 is B-Value
       ...
       # pad the embedding position info of subject, predicate and object
        ...
        ner_input = tf.reshape(ner_input,[true_batch_size,-1,max_span_size*3]) # shape [true_batch_size,max_num_spo,max_span_size*3]
